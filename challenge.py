...
name = input("Please enter an IP address: ")
if name[-1] != '.':
    name = name + '.'
dotCount = 1
numberCount = 0
for i in range(0, len(name)):
    if name[i] in '.':
        print("Segment {} contains {} characters".format(dotCount, numberCount))
        dotCount = dotCount + 1
        numberCount = 0
    else:
...
# The above method throws error when we press enter without typing anything.
